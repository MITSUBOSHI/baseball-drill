...
export function DrillDetail({ drill, category, categoryName }: { drill: Drill; category: string; categoryName: string }) {
  return <article className="drill-detail inner-page">
    <header className="detail-header"><nav><Link href="/">HOME</Link><span>/</span><Link href={`/drills/${category}`}>{categoryName}</Link></nav><div className="drill-tags"><span>{LEVEL_LABELS[drill.level]}</span>{drill.position && <span>{POSITION_LABELS[drill.position]}</span>}</div><h1><FuriganaText text={drill.title}/></h1><p><FuriganaText text={drill.description}/></p><div className="detail-meta"><span>◷ 所要時間　<b>{drill.duration}</b></span><span>参加人数　<b>{drill.players}</b></span><FavoriteButton drillId={drill.id}/></div></header>
